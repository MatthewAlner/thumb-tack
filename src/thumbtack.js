export default class ThumbTack {
  constructor(container, render = null, parent = null) {
    ...
    this.noderegister = {};
    this.render = render;
    this.thumb-tack = { "thumb-tack": { "Home": { "data": {} }}};
  ...
  start () {
    ...
    this.container.classList.add("parent-thumb-tack");
    this.container.tabIndex = 0;
    this.precanvas = document.createElement('div');
    this.precanvas.classList.add("thumb-tack");
  ...
  /* End Mobile Zoom */
  load() {
    for (var key in this.thumb-tack.thumb-tack[this.module].data) {
      this.addNodeImport(this.thumb-tack.thumb-tack[this.module].data[key], this.precanvas);
    }

    if(this.reroute) {
      for (var key in this.thumb-tack.thumb-tack[this.module].data) {
        this.addRerouteImport(this.thumb-tack.thumb-tack[this.module].data[key]);
      }
    }
    ...
    const editor = this.thumb-tack.thumb-tack;
    let number = 1;
  ...
  click(e) {
    ...
    if(this.editor_mode === 'fixed') {
      //return false;
       e.preventDefault();
       if(e.target.classList[0] === 'parent-thumb-tack' || e.target.classList[0] === 'thumb-tack') {
         this.ele_selected = e.target.closest(".parent-thumb-tack");
       } else {
         return false;
       }
    } else if(this.editor_mode === 'view') {
      if(e.target.closest(".thumb-tack") != null || e.target.matches('.parent-thumb-tack')) {
        this.ele_selected = e.target.closest(".parent-thumb-tack");
        e.preventDefault();
      }
    } else {
      ...
      if(e.target.closest(".thumb-tack_content_node") != null) {
        this.ele_selected = e.target.closest(".thumb-tack_content_node").parentElement;
      }
    }
    switch (this.ele_selected.classList[0]) {
      case 'thumb-tack-node':
  ...
  position(e) {
    ...
    if(this.drag) {
      ...
      this.thumb-tack.thumb-tack[this.module].data[this.ele_selected.id.slice(5)].pos_x = (this.ele_selected.offsetLeft - x);
      this.thumb-tack.thumb-tack[this.module].data[this.ele_selected.id.slice(5)].pos_y = (this.ele_selected.offsetTop - y);
    ...
    if(this.drag_point) {
      ...
      const searchConnection = this.thumb-tack.thumb-tack[this.module].data[nodeId].outputs[output_class].connections.findIndex(function(item,i) {
        return item.node ===  nodeUpdateIn && item.output === input_class;
      });

      this.thumb-tack.thumb-tack[this.module].data[nodeId].outputs[output_class].connections[searchConnection].points[numberPointPosition] = { pos_x: pos_x, pos_y: pos_y };
  ...
  dragEnd(e) {
    ...
    if(this.connection === true) {
      if(ele_last.classList[0] === 'input' || (this.force_first_input && (ele_last.closest(".thumb-tack_content_node") != null || ele_last.classList[0] === 'thumb-tack-node'))) {

        if(this.force_first_input && (ele_last.closest(".thumb-tack_content_node") != null || ele_last.classList[0] === 'thumb-tack-node')) {
          if(ele_last.closest(".thumb-tack_content_node") != null) {
            var input_id = ele_last.closest(".thumb-tack_content_node").parentElement.id;
          } else {
       ...
       var output_class = this.ele_selected.classList[1];
        ...
        if(output_id !== input_id && input_class !== false) {
          ...
          this.thumb-tack.thumb-tack[this.module].data[id_output].outputs[output_class].connections.push( {"node": id_input, "output": input_class});
          this.thumb-tack.thumb-tack[this.module].data[id_input].inputs[input_class].connections.push( {"node": id_output, "input": output_class});
  ...
  contextmenu(e) {
    ...
    if(this.precanvas.getElementsByClassName("thumb-tack-delete").length) {
      this.precanvas.getElementsByClassName("thumb-tack-delete")[0].remove()
    };
    if(this.node_selected || this.connection_selected) {
      var deletebox = document.createElement('div');
      deletebox.classList.add("thumb-tack-delete");
  ...
  addConnection(id_output, id_input, output_class, input_class) {
    ...
    if(nodeOneModule === nodeTwoModule) {
      ...
      if(exist === false) {
        //Create Connection
        this.thumb-tack.thumb-tack[nodeOneModule].data[id_output].outputs[output_class].connections.push( {"node": id_input.toString(), "output": input_class});
        this.thumb-tack.thumb-tack[nodeOneModule].data[id_input].inputs[input_class].connections.push( {"node": id_output.toString(), "input": output_class});
  ...
  createReroutePoint(ele) {
      ...
      if(this.thumb-tack.thumb-tack[this.module].data[nodeId].outputs[output_class].connections[searchConnection].points === undefined)  {
        this.thumb-tack.thumb-tack[this.module].data[nodeId].outputs[output_class].connections[searchConnection].points = [];
      }

      if(this.reroute_fix_curvature) {

        if(position_add_array_point > 0 || this.thumb-tack.thumb-tack[this.module].data[nodeId].outputs[output_class].connections[searchConnection].points !== []) {
          this.thumb-tack.thumb-tack[this.module].data[nodeId].outputs[output_class].connections[searchConnection].points.splice(position_add_array_point, 0, { pos_x: pos_x, pos_y: pos_y });
        } else {
          this.thumb-tack.thumb-tack[this.module].data[nodeId].outputs[output_class].connections[searchConnection].points.push({ pos_x: pos_x, pos_y: pos_y });
        }
  ...
  removeReroutePoint(ele) {
    ...
    this.thumb-tack.thumb-tack[this.module].data[nodeId].outputs[output_class].connections[searchConnection].points.splice(numberPointPosition,1);
  ...
  getNodeFromId(id) {
    ...
    return JSON.parse(JSON.stringify(this.thumb-tack.thumb-tack[moduleName].data[id]));
  }
  ...
  addNode (name, num_in, num_out, ele_pos_x, ele_pos_y, classoverride, data, html, typenode = false) {
    ...
    node.classList.add("thumb-tack-node");
    if(classoverride != '') {
    ...
    content.classList.add("thumb-tack_content_node");
    if(typenode === false) {
    ...
    this.thumb-tack.thumb-tack[this.module].data[newNodeId] = json;
  ...
  updateNodeValue(event) {
    ...
    for (var i = 0; i < attr.length; i++) {
            if (attr[i].nodeName.startsWith('df-')) {
                ...
                var target = this.thumb-tack.thumb-tack[this.module].data[event.target.closest(".thumb-tack_content_node").parentElement.id.slice(5)].data;
                ...
                this.dispatch('nodeDataChanged', event.target.closest(".thumb-tack_content_node").parentElement.id.slice(5));
          }
    }
  }
  ...
  updateNodeDataFromId(id, data) {
    var moduleName = this.getModuleFromNodeId(id)
    this.thumb-tack.thumb-tack[moduleName].data[id].data = data;
  ...
  addNodeInput(id) {
    ...
    this.thumb-tack.thumb-tack[moduleName].data[id].inputs["input_"+(numInputs+1)] = { "connections": []};
  }
  ...
  addNodeOutput(id) {
    ...
    this.thumb-tack.thumb-tack[moduleName].data[id].outputs["output_"+(numOutputs+1)] = { "connections": []};
  }
  ...
  removeNodeInput(id, input_class) {
    ...
    delete this.thumb-tack.thumb-tack[moduleName].data[id].inputs[input_class];

    // Update connection
    const connections = [];
    const connectionsInputs = this.thumb-tack.thumb-tack[moduleName].data[id].inputs
    Object.keys(connectionsInputs).map(function(key, index) {
      connections.push(connectionsInputs[key]);
    });
    this.thumb-tack.thumb-tack[moduleName].data[id].inputs = {};
    ...
    connections.forEach((item, i) => {
      ...
      this.thumb-tack.thumb-tack[moduleName].data[id].inputs['input_'+ (i+1)] = item;
    });
    ...
    nodeUpdates.forEach((itemx, i) => {
      this.thumb-tack.thumb-tack[moduleName].data[itemx.node].outputs[itemx.input].connections.forEach((itemz, g) => {
          if(itemz.node == id) {
            ...
            if(parseInt(input_class_id) < parseInt(output_id)) {
              ...
              if(itemz.points) {
                  this.thumb-tack.thumb-tack[moduleName].data[itemx.node].outputs[itemx.input].connections[g] = { node: itemz.node, output: 'input_'+(output_id-1), points: itemz.points }
              } else {
                  this.thumb-tack.thumb-tack[moduleName].data[itemx.node].outputs[itemx.input].connections[g] = { node: itemz.node, output: 'input_'+(output_id-1)}
              }
            }
          }
      });
  ...
  removeNodeOutput(id, output_class) {
    ...
    delete this.thumb-tack.thumb-tack[moduleName].data[id].outputs[output_class];

    // Update connection
    const connections = [];
    const connectionsOuputs = this.thumb-tack.thumb-tack[moduleName].data[id].outputs
    Object.keys(connectionsOuputs).map(function(key, index) {
      connections.push(connectionsOuputs[key]);
    });
    this.thumb-tack.thumb-tack[moduleName].data[id].outputs = {};
    ...
    connections.forEach((item, i) => {
      ...
      this.thumb-tack.thumb-tack[moduleName].data[id].outputs['output_'+ (i+1)] = item;
    });
    nodeUpdates =  new Set(nodeUpdates.map(e => JSON.stringify(e)));
    nodeUpdates = Array.from(nodeUpdates).map(e => JSON.parse(e));
    ...
    nodeUpdates.forEach((itemx, i) => {
      this.thumb-tack.thumb-tack[moduleName].data[itemx.node].inputs[itemx.output].connections.forEach((itemz, g) => {
          if(itemz.node == id) {
            ...
            if(parseInt(output_class_id) < parseInt(input_id)) {
              ...
              if(itemz.points) {
                  this.thumb-tack.thumb-tack[moduleName].data[itemx.node].inputs[itemx.output].connections[g] = { node: itemz.node, input: 'output_'+(input_id-1), points: itemz.points }
              } else {
                  this.thumb-tack.thumb-tack[moduleName].data[itemx.node].inputs[itemx.output].connections[g] = { node: itemz.node, input: 'output_'+(input_id-1)}
              }
            }
          }
      });
  ...
  removeNodeId(id) {
    ...
    delete this.thumb-tack.thumb-tack[moduleName].data[id.slice(5)];
  ...
  removeConnection() {
    if(this.connection_selected != null) {
      ...
      var index_out = this.thumb-tack.thumb-tack[this.module].data[listclass[2].slice(14)].outputs[listclass[3]].connections.findIndex(function(item,i) {
        return item.node === listclass[1].slice(13) && item.output === listclass[4]
      });
      this.thumb-tack.thumb-tack[this.module].data[listclass[2].slice(14)].outputs[listclass[3]].connections.splice(index_out,1);

      var index_in = this.thumb-tack.thumb-tack[this.module].data[listclass[1].slice(13)].inputs[listclass[4]].connections.findIndex(function(item,i) {
        return item.node === listclass[2].slice(14) && item.input === listclass[3]
      });
      this.thumb-tack.thumb-tack[this.module].data[listclass[1].slice(13)].inputs[listclass[4]].connections.splice(index_in,1);
  ...
  removeSingleConnection(id_output, id_input, output_class, input_class) {
    ...
    if(nodeOneModule === nodeTwoModule) {
      ...
      // Check connection exist
      var exists = this.thumb-tack.thumb-tack[nodeOneModule].data[id_output].outputs[output_class].connections.findIndex(function(item,i) {
      ...
      if(exists > -1) {
        ...
        var index_out = this.thumb-tack.thumb-tack[nodeOneModule].data[id_output].outputs[output_class].connections.findIndex(function(item,i) {
          return item.node == id_input && item.output === input_class
        });
        this.thumb-tack.thumb-tack[nodeOneModule].data[id_output].outputs[output_class].connections.splice(index_out,1);

        var index_in = this.thumb-tack.thumb-tack[nodeOneModule].data[id_input].inputs[input_class].connections.findIndex(function(item,i) {
          return item.node == id_output && item.input === output_class
        });
        this.thumb-tack.thumb-tack[nodeOneModule].data[id_input].inputs[input_class].connections.splice(index_in,1);
  ...
  addModule(name) {
    this.thumb-tack.thumb-tack[name] =  { "data": {} };
  ...
  changeModule(name) {
    ...
    this.import(this.thumb-tack, false);
  }
  ...
  removeModule(name) {
    ...
    delete this.thumb-tack.thumb-tack[name];
    this.dispatch('moduleRemoved', name);
  }

  clearModuleSelected() {
    this.precanvas.innerHTML = "";
    this.thumb-tack.thumb-tack[this.module] =  { "data": {} };
  }
  ...
  export () {
    const dataExport = JSON.parse(JSON.stringify(this.thumb-tack));
  ...
  import (data, notifi = true) {
    this.clear();
    this.thumb-tack = JSON.parse(JSON.stringify(data));
    this.load();
    if(notifi) {
